...
```bash
python3 scripts/line/make-richmenu-images.py    # 需要 pillow
```

會直接輸出兩張 2500×1686 的 PNG,格線與下面的座標表 px 級對齊,中文用系統
CJK 字型渲染(不會缺筆畫)。改文案就改腳本裡的 `TABS` 再重跑。
**測試階段用這個就夠了**;下面的 AI 生圖流程留給之後要做更精緻視覺時用。
